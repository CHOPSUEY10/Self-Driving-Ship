...
def get_curve(img, leftx, rightx):
    ...
    right_curverad = ((1 + (2 * right_fit_cr[0] * y_eval * ym_per_pix + right_fit_cr[1]) ** 2) ** 1.5) / np.absolute(
        2 * right_fit_cr[0])
    # Mengambil posisi horizontal tengah gambar (posisi mobil pada gambar).
    ...
    # Menghitung seberapa jauh posisi mobil dari tengah lajur, dikonversi ke meter (dan dibagi 10, kemungkinan untuk normalisasi).
    center = (car_pos - lane_center_position) * xm_per_pix 
    # Mengembalikan tuple yang berisi radius kelengkungan kiri, kanan, dan posisi relatif mobil terhadap tengah lajur.
...
def detect_object(frame) :
    ...
    with torch.no_grad():
        ...
        pred = non_max_suppression(pred, conf_thres=0.65, iou_thres=0.35)
    ...
    # Debug info
    print(f"[DEBUG] red_buoy: {len(red_centers)}, green_buoy: {len(green_centers)}, total boxes: {total_boxes}")
...
while True:
    ...
    cv.putText(kp, 'Steering Control', (int(w*0.4), int(h*0.81)), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv.putText(kp, '.', (int(lane_center_pos), int(h*0.89)), cv.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 25)
    offset = int((w//2) - lane_center_pos)
    steering_commands, side, PWM = calc_servo_pos(offset)
    cv.putText(kp, f"Remarks: {steering_commands} ", (10,80), cv.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
    print(steering_commands)
    cv.putText(kp, f"Servo Commands: SIDE={side} PWM(DC)={PWM}% ", (10,100), cv.FONT_HERSHEY_SIMPLEX, 0.4,
        (0, 255, 255), 1)
